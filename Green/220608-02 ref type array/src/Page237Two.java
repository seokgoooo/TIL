...
public class Page237Two {
	public static void main(String[] args) {
		...
		int[] num = new int[10];			// 입력 받은 정수를 넣어줄 배열
		int[] count = new int[10];			// range ~ range + 9 까지 범위의 정수 개수를 넣어줄 배열
		int check = 0;					// 정수 개수 배열의 index를 나타내줄 변수
		int range = 0;					// 범위를 나타내줄 변수

		// 정수가 범위에 맞는지 확인하는 반복문
		for (int i = 0; i < num.length; i++) {				// 배열 num 의 길이까지 반복해서 입력
			num[i] = scan.nextInt();

			while (range < 100) {					// 91 ~ 100까지가 마지막이기 때문에 range가 100 보다 작을 때 까지 실행
				if (num[i] > range && num[i] < range + 11) {	// range 초기값은 0. num[0] 가 0 보다 크고 11 보다 작을 때. 즉 1 ~ 10 까지의 범위
					count[check]++;				// 범위 안에 있으면 count[0] 의 값을 1 증가
					break;					// num[0] 의 범위를 찾았기 때문에 break로 반복문 종료
				} else {
					check++;				// range 범위에 있지 않은 경우 check 값을 1 증가 시켜 count[1] 의 개수를 확인
		...
		for (int i = 0; i < count.length; i++) {			// count 배열 길이까지 반복
			System.out.printf("%d ~ %d: ", range + 1, range + 10);	// 위 반복문에서 range = 0 으로 초기화. 여기서는 1 ~ 10: 이란 문자열 출력
			for (int j = 0; j < count[i]; j++) {			// count[i] 값 만큼 * 반복해서 출력
				System.out.print("*");
			}
