...
>>>>	그래서 조건에 따라 알아서 실행하는 반복문인 while for 를 사용한다.
	...
	2-2. boolean 변수로 스위치를 만드는 방법이 편하고 이해하기도 좋아서 이 프로그램에도 써보려고 처음엔 while ( ) 로 스위치를 만들어 줬는데 완성시켜보고 나니 if 에도 스위치를 넣어줄 수가 있다. 반복할 내용이 없으면 while 이 불필요하다.
	
	<<<기본 프로그램 진행 흐름은 위에서 아래로 내려가는 흐름이다.>>>
	if 를 통해서 input 이 들어오고
	반복문인 for 에서 수식대로 반복을 하면서 소수가 아닌 경우를 확인하는 과정을 거치는데
	소수가 아닌 경우에는 스위치를 꺼줘서 소수인 경우에 if 문에 흐름이 들어가지 않게 하고 break 로 찾은 즉시 반복문을 탈출하게 만들어서 종료 시킨다.
